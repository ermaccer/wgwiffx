...
int decompress_rnc(unsigned char* compressedBuff, int compressedSize, std::unique_ptr<unsigned char[]>* rawBuff, int* rawSize)
{
    ...
    v->file_size = compressedSize;
    v->input = (uint8*)compressedBuff;

    v->output = (uint8*)malloc(MAX_BUF_SIZE);
    v->temp = (uint8*)malloc(MAX_BUF_SIZE);
    int error_code = do_unpack(v);

    if (error_code == 0 && v->output)
    {
        *rawBuff = std::make_unique<unsigned char[]>(v->output_offset);
         memcpy(rawBuff->get(), v->output, v->output_offset);
        *rawSize = v->output_offset;
    }
